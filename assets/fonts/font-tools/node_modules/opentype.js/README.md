...
* `y`: Vertical position of the _baseline_ of the text. (default: `0`)
...
Fills a set of palette colors (from a palette index, or a provided array of CPAL color values) with a set of colors, falling back to the default color value, until a given count. _It does not modify the existing palette, returning a new array instead!_ Use `Font.palettes.setColor()` instead if needed.
...
* `y`: Vertical position of the _baseline_ of the glyph. (default: `0`)
